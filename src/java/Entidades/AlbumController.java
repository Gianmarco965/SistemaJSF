...
public class AlbumController implements Serializable {
    ...
    @EJB
    private Entidades.AlbumFacade ejbFacade;
    ...
    public Album getSelected() {
        if (current == null) {
            current = new Album();
            current.setAlbumPK(new Entidades.AlbumPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            current.getAlbumPK().setIdartista(current.getArtista().getIdartista());
            current.getAlbumPK().setIddisquera(current.getDisquera().getIddisquera());
    ...
    public Album getAlbum(Entidades.AlbumPK id) {
    ...
    public static class AlbumControllerConverter implements Converter {
        ...
        Entidades.AlbumPK getKey(String value) {
            Entidades.AlbumPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new Entidades.AlbumPK();
            key.setIdalbum(Integer.parseInt(values[0]));
            key.setIddisquera(Integer.parseInt(values[1]));
            key.setIdartista(Integer.parseInt(values[2]));
            return key;
        }

        String getStringKey(Entidades.AlbumPK value) {
            StringBuilder sb = new StringBuilder();
            sb.append(value.getIdalbum());
            sb.append(SEPARATOR);
            sb.append(value.getIddisquera());
            sb.append(SEPARATOR);
            sb.append(value.getIdartista());
            return sb.toString();
        }
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof Album) {
                ...
                return getStringKey(o.getAlbumPK());
            } else {
